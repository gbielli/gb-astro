...
Prenons une configuration server-side optimisée, dans le cas des requêtes envoyés au serveur et en présence d'un identifiant publicitaire présent dans l'url (gclid), on peut retrouver en réponse de celles-ci, des cookies publicitaires Google Ads déposés par le serveur :
...
Si ce cookie match les premiers octets de l'IP du domaine principal, alors aucune restriction ne sera appliqué par Safari. Et si ce n'est pas le cas, un mécanisme de réstoration de cookie (cookie restore) permettra quand même de restaurer ce dernier.
...
Vous continuerez donc à transmettre des requêtes tiers directement à Meta, TikTok, Pinterest, LinkedIn, etc.
