...
Entre l'Intelligence Tracking Prevention (ITP) de Safari, la montée en puissance des adblockers et la disparition progressive des cookies tiers, la contrainte sur la collecte de données ne cesse de s'intensifier.
...
- Les requêtes first-party peuvent lire et déposer des cookies sur votre domaine.
...
Le first-party est avant tout une porte d'entrée vers un tracking server-side, mais pas une fin en soi. Et je vais vous expliquer pourquoi.
...
Prenons une configuration server-side optimisée : en réponse aux requêtes envoyées à notre serveur, on peut retrouver des cookies publicitaires Google Ads déposés directement par le serveur, par exemple :

```
Set-Cookie: FPGCLAW=2.1.k1234$i1779457386; Max-Age=7776000; Domain=cabaia.fr; Path=/; Secure

```

À date, dans le cas du Google Tag Gateway, aucun cookie n'est déposé par le serveur, ni pour Google Ads, ni pour Google Analytics. Ce qui ne permet pas à GTG de profiter du prolongement de la durée de vie des cookies sur Safari.
...
Si on peut considérer, de manière très générale et peu précise, que 10 % des utilisateurs sont équipés d'adblockers (le chiffre est discutable), GTG reste un sérieux manque à gagner.
...
Vous continuerez donc à transmettre des requêtes tierces directement à Meta, TikTok, Pinterest, LinkedIn, etc. sans passer par votre domaine.
...
Cela reste néanmoins une nouveauté intéressante, qui nécessite d'être suivie de près dans les années à venir, pour voir comment elle est amenée à évoluer.
