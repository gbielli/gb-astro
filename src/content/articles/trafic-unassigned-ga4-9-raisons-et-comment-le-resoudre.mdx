---
title: "Trafic unassigned GA4 : 9 raisons et comment le résoudre"
description: "Le trafic unassigned dans Google Analytics 4 est probablement l'une des problématiques les plus frustrantes. Je vous donne 9 raisons et je vous explique comment le résoudre."
...
## Qu'est-ce que le trafic unassigned sur GA4 et comment l'observer ?
...
## 9 raisons de trafic unassigned sur Google Analytics 4
...
- **Si le premier événement qui porte le paramètre '\_ss' n'est pas dédié à GA4** mais à un autre outil, alors le session_start sera manquant et générera du trafic unassigned.

### Raison 5 : Des requêtes parasites via le Measurement Protocol

Pour qu'une requête via le Measurement Protocol (MP) puisse passer, il faut **pouvoir identifier un utilisateur, sa session ainsi que le timestamp**. Si vous n'envoyez pas ces données, les requêtes risquent tout simplement de ne pas pouvoir être attribuables par GA4. Rapprochez-vous de votre équipe technique pour le vérifier.
...
- Pour Google Ads notamment, pensez à activer **l'auto-tagging + les UTM manuels**.

### Raison 8 : Utilisation des triggers d'audience

Il semblerait que **la fonctionnalité de déclencheurs d'audience puisse générer du trafic unassigned**. J'utilise le conditionnel puisque je n'ai jamais eu l'occasion de le vérifier par moi-même. Vous pouvez accéder aux audiences de votre admin GA4 pour le vérifier.

<Image src={audienceTriggerGa4} alt="audience trigger GA4" />

### Raison 9 : Les bugs GA4

Plus rarement, GA4 lui-même peut être source de problèmes. Par exemple, en novembre 2024, certaines propriétés ont connu des délais de traitement anormalement longs (plus de 4 jours au lieu des 24h habituelles).
